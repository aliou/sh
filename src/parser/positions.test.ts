...
describe("position tracking", () => {
  ...
  it("attaches pos/end to simple commands and words", () => {
    const { ast } = parse("foo bar");
    const stmt = expectDefined(ast.body[0]);
    ...
    const [foo, bar] = expectDefined(cmd.words);
    expect(expectDefined(foo).pos).toEqual<Pos>({ offset: 0, line: 1, col: 1 });
    expect(expectDefined(foo).end).toEqual<Pos>({ offset: 3, line: 1, col: 4 });
    expect(expectDefined(bar).pos).toEqual<Pos>({ offset: 4, line: 1, col: 5 });
    expect(expectDefined(bar).end).toEqual<Pos>({ offset: 7, line: 1, col: 8 });
  });

  it("tracks line and column across newlines", () => {
    const { ast } = parse("foo\nbar");
    const second = expectDefined(ast.body[1]);
  ...
  it("spans control flow statements from keyword to terminator", () => {
    ...
    const { ast } = parse(src);
    const wh = expectDefined(ast.body[0]).command as WhileClause;
  ...
  it("attaches pos/end to expansions inside words", () => {
    ...
    const expansion = expectDefined(word.parts[0]);
